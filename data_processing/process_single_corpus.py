...
'''
这个类的主要功能是：
1、处理Python和SQL两种类型的语料数据，根据问题ID区分为单候选问题和多候选问题，并保存处理结果
2、将未标记的单候选数据转换为带有标签的数据
具体包括加载数据、分割数据、标签化数据以及保存处理结果到文件中
'''
#从指定的pickle文件中加载数据并返回
def load_pickle(filename):
    with open(filename, 'rb') as f:
        data = pickle.load(f, encoding='iso-8859-1')
    return data

#根据问题ID将数据分为单候选问题和多候选问题,其中total_data:是指包含所有数据的列表。qids是指包含问题ID的列表
...
#处理staqc数据，根据问题ID判断单候选和多候选问题，并保存到不同的文件中。
# 其中filepath是指输入文件的路径。save_single_path是指保存单候选问题数据的文件路径。save_multiple_path是指保存多候选问题数据的文件路径。
...
#处理large数据，根据问题ID判断单候选和多候选问题，并保存到不同的文件中
# 其中filepath是指输入文件的路径。save_single_path是指保存单候选问题数据的文件路径。save_multiple_path是指保存多候选问题数据的文件路径。
...
#主程序入口，依次调用各函数处理staqc和large数据库的Python和SQL数据，并转换和保存处理结果

...
#---------------------子函数1：代码的规则--------------------
#使用预定义的扫描器对输入字符串进行标记化，利用正则表达式扫描器将输入的字符串进行标记化。返回标记化的结果列表
def tokenizeRegex(s):
...
class SqlangParser():
    ...
    #解析SQL中的字符串标记。如果启用了正则表达式，将字符串标记化，否则替换为占位符"CODSTR"
    ...
    #递归地遍历SQL标记树，重命名列名和表名，使其采用标准化的格式。同时，还将数值和十六进制常量替换为占位符
    ...
    #初始化SqlangParser类，并对输入SQL进行清理、解析和标记化处理
    ...
    #递归地标识SQL查询中的子查询,如果找到子查询则返回true，没有找到则返回false
    ...
    #遍历传入的 tokenList 的所有标记，以识别其中的 SQL 函数并标记 SQL 代码中的函数
    ...
    #遍历 tokenList 中的所有标记，然后识别并标记 SQL 语句中的表名。这个函数还处理了子查询中的表名标记管理
...
#############################################################################
#缩略词处理
#扩展英文句子中的常见缩写（如it's -> it is）即把缩写恢复成完整的词语，然后返回处理后的文本。
...
#获取词性
#将POS标签转换为wordnet支持的标签格式，然后返回WordNet支持的词性标注标签。
def get_wordpos(tag):
    if tag.startswith('J'):
        return wordnet.ADJ
...
#############################################################################
#过滤掉字符串中不常用的特殊符号，确保字符串不会因为这些符号在解析时出错
...
#过滤掉字符串中不常用的特殊符号，但保留如#、/、,、=等的一些符号以应对不同的场景，确保字符串不会因为这些符号在解析时出错
...
#用于解析自然语言查询，将其标准化并分词但对于过滤符号的要求稍低
